...
var ws;
function getWebSocket(tp,sendData){
	ws = new WebSocket('ws://10.13.91.222:1234');
	ws.onopen = function(msg){
	//console.log('Connection Success!');
	if (tp==1) ws.send('{"id":"1","uname":"'+uname+'"}');
	else if (tp==2) ws.send(sendData);
	else if (tp==5){
		try{
			ws.send(sendData);
		}catch(e){
	...
	ws.onmessage = function(msg){
		...
		if (ids=='1'){
			...
			starId=jsonData['starId'];
			ranY=jsonData['ranY'];
			ranX=jsonData['ranX'];
			starHtml='<div class="star heart" id="'+starId+'"></div>';
			$('.content').append(starHtml);
...
function addTime(){
	...
	if (timeNumb%6==0&&uname=='abc'){
		...
		sendData='{"id":"2","uname":"'+uname+'","starId":"'+starId+'","ranX":"'+ranX+'","ranY":"'+ranY+'"}';
		$('#'+starId).css({'top':ranY+'px','left':ranX+'px'});
		
		getWebSocket(2,sendData);
	}
