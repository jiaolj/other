...
hcountry={1:'美国',2:'泰国'}
def getHptName(cur,hid):
    cur.execute('select name from hospital where id=%s',[hid])
    r=cur.fetchone()
    if r:return r[0]
    ...
    if r:
        for n in r:
            ...
            for i,m in enumerate(n):
                dirs[cms[i]]=m
                if t=='dct' and i==2:dirs['hname']=getHptName(cur,m)
                elif t=='hpt' and i==2:dirs['cname']=hcountry[m]
            lst.append(dirs)
    return lst
#----去掉html标签，获得纯文本
