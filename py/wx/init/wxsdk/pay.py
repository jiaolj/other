...
class WxPayConf_pub(object):
    ...
    APPID = None # "wx8ca23ce160f7596a"
    #JSAPI接口中获取openid，审核后在公众平台开启开发模式后可查看
    APPSECRET = None # "592e15e0fc4e6c8de1d35942fe921770"

    #受理商ID，身份标识
    PARTNER_ID = None # "1224214301"
    #商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    PARTNER_KEY = None # "lPVyihZayCdzL5bpK4vrAfla9KdVQJ88"

    #=======【异步通知url设置】===================================
    #异步通知url，商户根据实际开发过程设定
    NOTIFY_URL = None #"http://m2.guonong.me/mobile/pay/wxpay_cb.htm"
    ...
    #=======【证书路径设置】=====================================
    #证书路径,注意应该填写绝对路径
    SSLCERT_PATH = "/home/lee/cacert/apiclient_cert.pem"
    SSLKEY_PATH =  "/home/lee/cacert/apiclient_key.pem"
    ...
    @classmethod
    def config(cls, app_id=None, app_secret=None, partner_id=None, partner_key=None, notify_url=None):
        cls.APPID = app_id
        cls.APPSECRET = app_secret

        cls.PARTNER_ID = partner_id
        cls.PARTNER_KEY = partner_key

        cls.NOTIFY_URL = notify_url

    @classmethod
    def to_string(cls):
        return "WxPayConf_pub[app_id:%s, app_secret:%s, partner_id:%s, partner_key:%s]" % (cls.APPID, cls.APPSECRET, cls.PARTNER_ID, cls.PARTNER_KEY)
...
class Common_util_pub(object):
    ...
    def getSign(self, obj):
        ...
        #签名步骤二：在string后加入KEY
        String = "{0}&key={1}".format(String,WxPayConf_pub.PARTNER_KEY)
        #签名步骤三：MD5加密
    ...
    def arrayToXml(self, arr):
        ...
        for k, v in arr.iteritems():
            if isinstance(v, (int, long)) or v.isdigit():
...
class Wxpay_client_pub(Common_util_pub):
    ...
    def createXml(self):
        ...
        self.parameters["mch_id"] = WxPayConf_pub.PARTNER_ID   #商户号
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def getPrepayId(self):
        ...
        self.postXml()
        self.result = self.xmlToArray(self.response)
        print self.result, self.result.get('return_msg'),
        print WxPayConf_pub.to_string()
...
class OrderQuery_pub(Wxpay_client_pub):
    ...
    def createXml(self):
        ...
        if not self.parameters.has_key('out_trade_no'):
            raise ValueError("missing parameter")
...
class NativeCall_pub(Wxpay_server_pub):
    ...
    def createXml(self):
        ...
        if self.returnParameters["return_code"] == self.SUCCESS:
            ...
            self.returnParameters["mch_id"] = WxPayConf_pub.PARTNER_ID #商户号
...
if __name__ == "__main__":
    WxPayConf_pub.config(
        app_id='wx60b34c2f3b272ccf',
        app_secret='51b347d1473994529b2ffb0ba5462e92',
        partner_id='1233929102',
        partner_key='EC3JrY0eO7D3zcZBgdVbZtOq28SkLu9v',
        notify_url='http://test.guonong.me/b/pay/wxpay_cb.htm'
    )
    # unified_order = UnifiedOrder_pub()
    # unified_order.setParameter('out_trade_no', '30033')
    # unified_order.setParameter('body','测试拉')
    # unified_order.setParameter('total_fee','1')
    # unified_order.setParameter('notify_url', WxPayConf_pub.NOTIFY_URL)
    # unified_order.setParameter('trade_type','JSAPI')
    # unified_order.setParameter('openid', 'ouKCzuPCdk4CSoYVJCgn6ubM4n4g')
    #
    # print unified_order.getPrepayId()
    #
    # jsapid_pub = JsApi_pub()
    # jsapid_pub.setPrepayId('wx20150207104605c5f07099fb0691448050')
    # print jsapid_pub.getParameters()

    order_query = OrderQuery_pub()
    order_query.setParameter('out_trade_no', '8120214')

    print order_query.getResult()
